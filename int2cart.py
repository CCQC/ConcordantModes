...
class int2cart(object):
    ...
    def run(self):
        ...
        self.Carts = np.array([[0.,0.,0.]])

        """ Second atom is placed along the x-axis. """
        self.Carts = np.append(self.Carts,np.array([[self.varDict[self.zmat.bondVariables[0]],0.,0.]]),axis=0)
        ...
        if len(self.zmat.atomList) > 2:
            r = self.Carts[int(self.zmat.angleIndices[0][2])-1]-self.Carts[int(self.zmat.angleIndices[0][1])-1]
            r /= np.linalg.norm(r)
            r *= self.varDict[self.zmat.bondVariables[1]]
            theta = self.varDict[self.zmat.angleVariables[0]]*np.pi/180.
            ang_rot = self.x_rot(theta)
            cart = np.dot(r,ang_rot)+self.Carts[int(self.zmat.angleIndices[0][1])-1]
