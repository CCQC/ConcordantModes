    ...
    def run(self):
        ...
        for i in range(n_disp - np.sum(self.dispSym)):
            ...
            if i == 0:
                ...
                print("Job number " + "{:5d}".format(i+1) + ": " 
                        + str(energy[0]))
        ...
        for i in range(len(self.Energies)):
            if i == 0:
                print("Reference energy: " + str(np.float(self.Energies[i])
                        - np.float(self.Energies[0])))
            else:
                print("Job number " + "{:5d}".format(i+1) + ": " 
                        + "{:+1.8f}".format(np.float(self.Energies[i])
                        - np.float(self.Energies[0])))
        ...
        for i in range(n_disp - np.sum(self.dispSym) - 1):
            ...
            k = Sum // 2
            if j == 0:
                self.energiesDict[str(k+1)+'_plus'] = float(
                        self.Energies[i+1])
            if j == 1:
                self.energiesDict[str(k+1)+'_minus'] = float(
                        self.Energies[i+1])
            # if self.dispSym[k] == 0:
                # if j == 0:
