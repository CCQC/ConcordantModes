...
class TransDisp(object):
    def __init__(
        self, s_vectors, zmat, disp, eigs, Conv, dispTol, TED, options, indices, GF=None
    ):
        """
    ...
    def run(self):
        ...
        for i in range(len(self.eig_inv)):
            ...
            self.eig_inv[i][
                np.abs(self.eig_inv[i]) < np.max(np.abs(self.eig_inv[i])) * projTol
            ] = 0
        ...
        self.A = self.ComputeA(
            self.s_vectors.B.copy(), self.TED.Proj, self.eig_inv, self.zmat.massWeight
        )
    ...
    def INTC(self, carts, eig_inv, Proj):
        ...
        for i in range(len(self.zmat.angleIndices)):
            ...
            if self.Conv:
                Condition1 = (
                    float(
                        self.zmat.variableDictionaryFinal[self.zmat.angleVariables[i]]
                    )
                    > 180.0
                )
                Condition2 = (
        ...
        for i in range(len(self.zmat.torsionIndices)):
            ...
            if self.Conv:
                Condition1 = float(
                    self.zmat.variableDictionaryFinal[self.zmat.torsionVariables[i]]
        ...
        for i in range(len(self.zmat.oopIndices)):
            ...
            if self.Conv:
                Condition1 = (
                    float(self.zmat.variableDictionaryFinal[self.zmat.oopVariables[i]])
                    > 180.0
                )
                Condition2 = (
                    float(self.zmat.variableDictionaryFinal[self.zmat.oopVariables[i]])
        ...
        for i in range(len(self.zmat.linIndices)):
            x1 = np.array(carts[int(self.zmat.linIndices[i][0]) - 1]).astype(float)
            x2 = np.array(carts[int(self.zmat.linIndices[i][1]) - 1]).astype(float)
            x3 = np.array(carts[int(self.zmat.linIndices[i][2]) - 1]).astype(float)
            x4 = np.array(carts[int(self.zmat.linIndices[i][3]) - 1]).astype(float)
    ...
    def CoordConvert(
        ...
        for i in range(max_iter):
            ...
            if tightDisp:
                ...
                A = self.ComputeA(
                    sVec.B, self.TED.Proj, self.eig_inv, self.zmat.massWeight
                )
