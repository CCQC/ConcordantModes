...
class ZMAT(object):
    ...
    def run(self):
        ...
        if self.options.coords.upper() == "ZMAT":
            ...
            for i in range(len(zmatOutput)):
                if re.search(bondRegex, zmatOutput[i]):
                    ...
                    blank += 1
                Sum = (
                    len(self.bondVariables)
                    + len(self.angleVariables)
                    + len(self.torsionVariables)
                    + len(self.oopVariables)
                    + len(self.linVariables)
                    + blank
                )
        ...
        indices = []
        transdisp = TransDisp(
            1, self, 1, 1, False, self.dispTol, np.array([]), self.options, indices
        )
        I = np.eye(
            len(self.bondIndices)
            + len(self.angleIndices)
            + len(self.torsionIndices)
            + len(self.oopIndices)
            + len(self.linIndices)
        )
        ...
        for i in range(
            ...
            variables1[len(self.bondIndices) + i] *= 180.0 / np.pi
            variables2[len(self.bondIndices) + i] *= 180.0 / np.pi
        Variables = np.append(
            self.bondVariables, np.append(self.angleVariables, self.torsionVariables)
        )
        ...
        for i in range(len(self.torsionVariables)):
            Condition1 = (
                float(self.variableDictionaryInit[self.torsionVariables[i]]) <= -90.0
            )
            Condition2 = (
                float(self.variableDictionaryInit[self.torsionVariables[i]]) >= 270.0
            )
            buff = np.floor(
                abs(float(self.variableDictionaryInit[self.torsionVariables[i]])) / 360
            )
            if Condition1:
                self.variableDictionaryInit[self.torsionVariables[i]] = float(
                    self.variableDictionaryInit[self.torsionVariables[i]]
                )
                self.variableDictionaryInit[self.torsionVariables[i]] += 360.0 * buff
                if (
                    float(self.variableDictionaryInit[self.torsionVariables[i]])
                    <= -90.0
                ):
                    self.variableDictionaryInit[self.torsionVariables[i]] += 360.0
            if Condition2:
                self.variableDictionaryInit[self.torsionVariables[i]] = float(
                    self.variableDictionaryInit[self.torsionVariables[i]]
                )
        ...
        for i in range(len(self.torsionVariables)):
            Condition1 = (
                float(self.variableDictionaryFinal[self.torsionVariables[i]]) <= -90.0
            )
            Condition2 = (
                float(self.variableDictionaryFinal[self.torsionVariables[i]]) >= 270.0
            )
            buff = np.floor(
                abs(float(self.variableDictionaryFinal[self.torsionVariables[i]])) / 360
            )
            if Condition1:
                self.variableDictionaryFinal[self.torsionVariables[i]] = float(
                    self.variableDictionaryFinal[self.torsionVariables[i]]
                )
                self.variableDictionaryFinal[self.torsionVariables[i]] += 360.0 * buff
                if (
                    float(self.variableDictionaryFinal[self.torsionVariables[i]])
                    <= -90.0
                ):
                    self.variableDictionaryFinal[self.torsionVariables[i]] += 360.0
            if Condition2:
                self.variableDictionaryFinal[self.torsionVariables[i]] = float(
                    self.variableDictionaryFinal[self.torsionVariables[i]]
                )
        ...
        for i in range(len(Variables)):
            print(
                Variables[i] + " = " + str(self.variableDictionaryFinal[Variables[i]])
            )
            # if self.options.coords.upper() == "REDUNDANT":
            # print(indices[i])
        ...
        for i in range(len(Variables)):
            print(
                ...
                + " = "
                + str(
                    self.variableDictionaryFinal[Variables[i]]
                    - self.variableDictionaryInit[Variables[i]]
                )
            )
            # if self.options.coords.upper() == "REDUNDANT":
            # print(indices[i])
